...
import { Bell, CircleDollarSign, LoaderCircle, TrendingUp, UserRoundPlus, WalletCards, X } from "lucide-react";
import { useCallback, useEffect, useRef, useState } from "react";
import { adminNotificationResponseSchema, type AdminNotificationResponse } from "@/features/admin/notifications/schema";

type Notification = AdminNotificationResponse["items"][number];
...
export function AdminNotifications({ initialCount }: { initialCount: number }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        const parsed=adminNotificationResponseSchema.safeParse(await response.json());
        if(!parsed.success){setError(true);return;}
        const data:AdminNotificationResponse=parsed.data;
        setCount(data.count); setItems(data.items); setError(false);
      } else setError(true);
  ...
  const icon = (type: Notification["type"]) => {
    if (type === "MEMBER") return <UserRoundPlus className="size-4" />;
    if (type === "DEPOSIT") return <CircleDollarSign className="size-4" />;
    if (type === "WITHDRAWAL") return <WalletCards className="size-4" />;
  ...
  return <div ref={root} className="relative">
    ...
    {open && <div className="fixed inset-x-3 top-[74px] z-50 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl sm:absolute sm:inset-x-auto sm:right-0 sm:top-12 sm:w-[390px]">
      <div className="flex items-center justify-between border-b border-slate-100 p-4"><div><p className="font-bold">Admin notifications</p><p className="text-xs text-slate-500">{count} items require attention</p></div><button aria-label="Close notifications" onClick={() => setOpen(false)} className="grid size-8 place-items-center rounded-lg hover:bg-slate-100"><X className="size-4" /></button></div>
      {error&&<div className="border-b border-red-100 bg-red-50 px-4 py-2 text-xs text-red-700">Could not refresh the work queue. Existing items may be out of date.</div>}
      <div className="max-h-[min(65vh,520px)] overflow-y-auto">{loading && items.length === 0 ? <div className="grid h-32 place-items-center"><LoaderCircle className="size-5 animate-spin text-emerald-500" /></div> : items.length === 0 ? <p className="p-8 text-center text-sm text-slate-500">No recent or pending notifications.</p> : items.map((item) => <Link key={item.id} href={item.href} onClick={() => setOpen(false)} className="flex gap-3 border-b border-slate-100 p-4 transition last:border-0 hover:bg-emerald-50/50"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-700">{icon(item.type)}</span><span className="min-w-0"><strong className="block text-sm">{item.title}</strong><span className="block truncate text-xs text-slate-500">{item.detail}</span><span className="mt-1 block text-[10px] text-slate-400">{new Date(item.createdAt).toLocaleString()}</span></span></Link>)}</div>
