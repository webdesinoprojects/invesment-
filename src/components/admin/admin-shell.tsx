...
export function AdminShell({ session, pendingActions, children }: { session: AdminSession; pendingActions: number; children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#f3f6f5] text-slate-950">
    ...
    <div className={`transition-[padding] duration-300 ${desktopSidebarCollapsed ? "lg:pl-20" : "lg:pl-64"}`}><AdminHeader session={session} pendingActions={pendingActions} onMenuOpen={() => setMobileMenuOpen(true)} /><main className="p-3 sm:p-4 md:p-7">{children}</main></div>
  </div>;
}
