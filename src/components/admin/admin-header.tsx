...
export function AdminHeader({ session, pendingActions, onMenuOpen }: { session: AdminSession; pendingActions: number; onMenuOpen: () => void }) {
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-2 border-b border-slate-200 bg-white/95 px-3 backdrop-blur-xl sm:gap-3 sm:px-4 lg:h-20 lg:px-7">
      ...
      <div className="hidden lg:block"><p className="text-sm font-bold text-slate-800">NaturePower</p></div>
      ...
      <div className="text-right leading-tight lg:hidden"><p className="text-xs font-bold text-slate-800">NaturePower</p><p className="text-[9px] font-bold uppercase tracking-wider text-emerald-600">Admin</p></div>
